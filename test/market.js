/* Market page (city-only trade): sell at worth, buy at worth *
   ...
   (G.zoneHasMarket), not a one-time Khar-Barak unlock; Aerendell is
   itself a City, so the Market is available from a fresh save.
...
G.wipe(); S.weight = 0;
console.log('  aerendell (a City) allows trade:', G.zoneHasMarket() === true);
...
G.wipe(); S.weight = 0;
UI.renderAll();
console.log('  shown in aerendell (a City):', document.getElementById('market-btn').style.display === 'flex');
S.zone = 'forestRoad';
UI.renderAll();
